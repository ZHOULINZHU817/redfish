<template>
	<div class="address">
		...
		<div class="address-nulls" v-else><img :src="nullImg" /></div>
	</div>
</template>
...
export default {
	...
	methods: {
			...
			wx.showModal({
				...
				success(res) {
					if (res.confirm) {
						...
						if ($storage.get('changeaddress') && item.addressId == $storage.get('changeaddress').addressId) {
							$storage.remove('changeaddress');
						}
		...
		changeAddress(str) {
			$storage.set('changeaddress', str);
			if (this.getJson == 1) {
				$router.back();
			}
		},
		addAddressClick() {
			$router.push('user_modules/address/add');
		},
		editAddress(item) {
			console.log(11111);
			this.$state.set('addressId', item.addressId);
			$router.push('user_modules/address/edit');
		}
	}
};
</script>
