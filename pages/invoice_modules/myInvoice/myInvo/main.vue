		...
		<div class="myInvoice-con">
			<ul v-if="Invos.length > 0">
				<li v-for="(invo, invoIndex) in Invos" :key="invoIndex">
					<h1>
						开票时间:&nbsp;&nbsp;{{ invo.gmtModified }}
						<span>{{ invo.dataState == '0' ? '已提交' : invo.dataState == '1' ? '已开票' : '开票失败' }}</span>
					</h1>
...
export default {
	...
	onLoad(options) {
		...
		this.$state.moreToolList = [...this.$state.moreToolList, ...this.$state.currentRouter.routechildren]
	...
	mounted() {
		...
		this.$state.moreToolList.map(v => {
			...
			if (v.menuAction == 'issueInvoice') {
				this.issueInvoice = v.menuJspath;
			}
			if (v.menuAction == 'addInfo') {
				this.addInfo = v.menuJspath;
			}
			if (v.menuAction == 'addInvoice') {
				this.addInvoiceMenu = v.menuJspath;
			}
			...
			http.get(invoice.checkAuditing).then(res => {
				if (res.dataState == '1') {
					...
					wx.showModal({
						...
						success(res) {
							if (res.confirm) {
								$router.push(that.addInvoiceMenu);
							} else if (res.cancel) {
								// console.log('用户点击取消')
							}
						}
