<template>
	<div class="warp-box">
		...
		<div @click="goBack" class="shopList-left">
			<i class="iconfont">&#xe62d;</i>
		</div>
		...
		<!-- 普通 -->
		<Specs :specsStrList="specsList" @showSpec="showSpec"></Specs>
		...
		<Footers :goodsCode="goodsCode" :indexs="indexs" :skuList="skuList" :userImgurl="userImgurl" :baseColor="baseColor"
		 @addcollectGoods="addcollectGoods" @getSpec="getSpec" @showSpec="showSpec" v-if="getgoodtypes == '00'"></Footers>
		<!-- 团购 -->
		<groupFooters :couponOnceNumd="couponOnceNumd" :getgoodtypes="getgoodtypes" :goodsCode="goodsCode" :indexs="indexs"
		 :skuList="skuList" :userImgurl="userImgurl" @addcollectGoods="addcollectGoods" @getSpec="getSpec" @showSpec="showSpec"
		 v-if="getgoodtypes == '24'"></groupFooters>
		<!-- 拼团 -->
		<assFooters :couponOnceNumd="couponOnceNumd" :couponOnceNums="couponOnceNums" :getgoodtypes="getgoodtypes" :goodsCode="goodsCode"
		...
		<!-- 秒杀 -->
		<seckillFooters :getgoodtypes="getgoodtypes" :goodsCode="goodsCode" :indexs="indexs" :skuList="skuList" :userImgurl="userImgurl"
		...
		<div :style="{bottom:bot}" class="spec-boxinfor">
			<div class="auto mar-t20 spec-ptn">
				<div class="spec-header t-box">
					<div class="spec-img mar-r20">
						<img :src="skuList.dataPic" id="goodsImg" />
					</div>
					<div class="t-box-flex1">
					...
					<div @click="_close()" class="spec-close">
						...
						<div :key="index" class="spec-list t-box" v-for="(item,index) in specList">
							...
							<ul :key="indexs" class="clearfix attribute-list" v-for="(items,indexs) in item.specOptionList">
								<li :style="{'color':(indexs === items.nu ?'#ffffff':''),'background':(indexs === items.nu ?baseColor:''),}"
								 @click="changespecList(indexs,index,items.specValueValue)">{{items.specValueValue}}</li>
							</ul>
						</div>
					</div>
				</div>
			...
			<div class="t-box spec-btn">
				<div :style="{background:baseColor}" @click="confirmBtn(1)" class="t-box-flex1 t-center surebtn">
					<a>确定</a>
				</div>
			</div>
		</div>
		...
		onShareAppMessage: function(res) {
			...
			if (res.from === "button") {
				...
				if ($storage.get("loginInfor").userinfoQuality.indexOf("group") >= 0) {
					_path =
						`/pages/o2o/pages/goodsdetails_modules/o2o_goosDetail/main?groupCode=${
          $storage.get("loginInfor").userInfoCode
        }&groupName=${$storage.get("loginInfor").merberCompname}&skuCode=${
          that.getJson
        }&contractTypepro=40`;
				} else {
					_path = `/pages/o2o/pages/goodsdetails_modules/o2o_goosDetail/main?skuCode=${
          that.getJson
        }`;
				}
				return {
			...
			getSkuInfo(codes) {
				http.get(getResourceGoodsInfoBySkuCode, {
					...
					if (res.goodsType == "25") {
						...
						http
							.get(queryPromotionPageAssembleForPc, {
					...
					if (res.rsSkuDomainList.length > 0) {
						...
						this.getspecs =
							this.skuList.skuName &&
							this.skuList.skuName.replace(new RegExp("/", "g"), ",");
					}
					if (this.skuList.goodsSalesvolume == null) {
						this.skuList.goodsSalesvolume = "0";
					}
					this.specsList =
						this.skuList.skuName && this.skuList.skuName.split("/");
					this.skuId = this.skuList && this.skuList.skuId;
					this.goodsPrice =
						this.skuList.pricesetNprice &&
						(this.skuList.pricesetNprice * 1).toFixed(2);
				...
				http.post(addShoppingGoodsBySpec, _data).then(res => {
					if (res.success) {
						...
						if (res.dataObj != null) {
							...
							if (
								this.getgoodtypes == 24 ||
								this.getgoodtypes == 25 ||
								this.getgoodtypes == 26
							) {
								this.goBuy();
							} else {
								this.addCart();
							...
							success(res) {
								http
									.post("/web/ml/mlogin/loginMiniCode.json", {
										...
										scene: `40-${$storage.get("loginInfor").userId}-${
                    that.getJson
                  }`, //contractTypepro-userId-skuCode
